...
{
    public partial class CyhkTcpClient : ClassBase<CyhkTcpClient>
    {
        #region 字段 
        ...
        private bool _isMobile; //是否移动用户登录
        ...
        private Task _tk_resolvermsg = null; //解析消息数据
        private Task _tk_resolvergpsmsg = null; //解析GPS消息数据
        ...
        /// 接收到OBD数据信息
        /// </summary>
        public event Action<string, Fields> ObdDataReceived;
        /// <summary>
        /// 接收到打捆机数据
        /// </summary>
        public event Action<string, Fields> DKJDataReceived;
        /// <summary>
        /// 接收到打捆机状态
        /// </summary>
        public event Action<string, Fields> DKJStateReceived;
        /// <summary>
        /// 接收到打捆机数据 新
        /// </summary>
        public event Action<string, Fields> DKJDataNewReceived;
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public void Init(string ip, string port, string username, string userpwd, string mac = null)
        {
            _ip = ip;
            ...
            WebTools.GET((x, y, z) =>
            {
                if (x && y == WebExceptionStatus.Success && z.code == 0)    //命令列表请求成功
                {
                    Command.cmdInfoList = JsonConvert.DeserializeObject<List<DICT_CMD>>(z.tag.ToString());
                    WebTools.GET((a, b, c) =>
                    {
                        if (a && b == WebExceptionStatus.Success && c.code == 0)    //命令错误状态列表请求成功
                        {
                            Command.cmdStatusList = JsonConvert.DeserializeObject<List<DICT_ERROR>>(c.tag.ToString());
                            inited = true;
                        }
                        else
                        {
        ...
        private void ConnectMethod(object state)
        {
            ...
            if (!_serviceClosed)
            {
                ...
                else
                {
                    //_state = new StateObject();
                    //未连接成功
                    bool suc = _tcpClient.ConnectServer(_ip, _port, _state);
                }
            }
            else
            {
        ...
        public bool StartService()
        {
            ...
            {
                _tk_dealrecdata = new Task(DealReceiveData, TaskCreationOptions.LongRunning);
            }
            if (_tk_dealrecdata.Status != TaskStatus.Running)
                _tk_dealrecdata.Start();

            if (_tk_resolvermsg == null)
            {
                _tk_resolvermsg = new Task(ResolveMessage, TaskCreationOptions.LongRunning);
            }
            if (_tk_resolvermsg.Status != TaskStatus.Running)
                _tk_resolvermsg.Start();

            if (_tk_resolvergpsmsg == null)
            {
                _tk_resolvergpsmsg = new Task(ResolveGpsMessage, TaskCreationOptions.LongRunning);
            }
            if (_tk_resolvergpsmsg.Status != TaskStatus.Running)
                _tk_resolvergpsmsg.Start();
        ...
        public bool SendMessage(ushort cmd_id, ICar car, byte[] message)
        {
            ...
            if (car != null)
            {
                mobileType = car.MTYPE;
                ciserverNo = car.CTYPE;
            }
        ...
        public bool SendBS(Byte[] data, int offset, int size, Command userToken = null)
        {
            ...
            try
            {
                ...
                if (suc)
                {
                    ...
                    //suc = sendResetEvent.WaitOne(new TimeSpan(0, 0, 20));
                    //if (!suc && userToken != null)
                    //{
                    //    userToken.AckCode = 60001;  //超时
                    //    InvokeCommandAckHandler(userToken);
                    //    cmdSendList.Remove(userToken); //移除超时命令 
                    //}
                }
                else
                {
        ...
        void DealReceiveData(object stat)
        {
            if (!_serviceClosed)
            {
                ...
                {
                    bool suc = _state.BufferQueue.EnqueueEvent.WaitOne(1000);
                    if (!suc && _state.BufferQueue.Count > 0)
                    {
        ...
        private readonly TQueue<byte[]> _msgQueue = new TQueue<byte[]>();
        /// <summary>
        /// 解析消息内容
        /// </summary>
        public void ResolveMessage()
        {
            while (!_serviceClosed)
            {
                bool suc = _msgQueue.EnqueueEvent.WaitOne(1000);
                if (!suc && _msgQueue.Count > 0)
                {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        try
                        {
                            byte[] msgBytes = _msgQueue.Dequeue();

                            //0-16位是消息头
                            ushort recCmdId = NetTools.NetworkToHostOrderFromBytesToUshort(msgBytes, 3);
                            ushort seqNum = NetTools.NetworkToHostOrderFromBytesToUshort(msgBytes, 5);
                            int messageIndex = 17; //从17位开始是消息内容
                            if (recCmdId == CommandID.GPS_PACKET) //GPS位置信息
                            {
                                _gpsMsgQueue.Enqueue(msgBytes);
                            }
                            ...
                            else if (recCmdId == CommandID.UP_MSG_GSYJ)
                            {
                                if (MessageRecived_YJGS != null) //调用事件
                                {
                                    ...
                                    Fields fds = new Fields(new[]
                                    {
                                        ...
                                        new Field(new Range(34, 4u), typeof(int), "msgLength"),
                                        new Field(new Range(38, msgBytes.Length - 1), typeof(byte[]), "msg"),
                                    }, msgBytes);
                            ...
                            #region OBD数据
                            else if (recCmdId == CommandID.OBD_DATA)
                            {
                                if (ObdDataReceived != null)
                                {
                                    string mac = NetTools.NetworkToHostOrderFromBytesToString(msgBytes, messageIndex, 16);
                                    Fields fds = new Fields(new[]
                                    {
                                        new Field(new Range(messageIndex, 16u), typeof(string), "mac"),
                                        new Field(new Range(messageIndex += 16, 2u), typeof(ushort), "OBDFactory",
                                            "厂家代码 0 元征（透明） 255终端解析"),
                                        new Field(new Range(messageIndex += 2, 12u), typeof(string), "Time",
                                            "Yymmddhh24miss"),
                                        new Field(new Range(messageIndex += 12, 2u), typeof(ushort), "dataLen",
                                            "OBD数据长度"),
                                        new Field(new Range(messageIndex += 2, msgBytes.Length - 1), typeof(byte[]),
                                            "data", "OBD数据内容 见附表（终端解析过），透明按原始OBD协议解析"),
                                    }, msgBytes);

                                    ObdDataReceived.Invoke(mac, fds);
                                }
                            }
                            #endregion

                            #region 打捆机数据
                            else if (recCmdId == CommandID.DKJ_DATA) //打捆机数据
                            {
                                if (DKJDataReceived != null)
                                {
                                    string mac = NetTools.NetworkToHostOrderFromBytesToString(msgBytes, messageIndex, 16);
                                    Fields fds = new Fields(new[]
                                    {
                                        new Field(new Range(messageIndex, 16u), typeof(string), "mac"),
                                        new Field(new Range(messageIndex += 16, 12u), typeof(string), "time"),
                                        new Field(new Range(messageIndex += 12, 2u), typeof(ushort), "data"),
                                    }, msgBytes);

                                    DKJDataReceived.Invoke(mac, fds);
                                }
                            }
                            else if (recCmdId == CommandID.DKJ_STATE) //打捆机状态
                            {
                                if (DKJStateReceived != null)
                                {
                                    string mac =
                                        NetTools.NetworkToHostOrderFromBytesToString(msgBytes, messageIndex, 16);
                                    Fields fds = new Fields(new[]
                                    ...
                                    DKJStateReceived.Invoke(mac, fds);
                                }
                            }
                            else if (recCmdId == CommandID.DKJ_DATA_NEW) //新打捆机数据
                            {
                                if (DKJDataNewReceived != null)
                                {
                                    string mac =
                                        NetTools.NetworkToHostOrderFromBytesToString(msgBytes, messageIndex, 16);
                                    Fields fds = new Fields(new[]
                                    {
                                        new Field(new Range(messageIndex, 16u), typeof(string), "mac"),
                                        new Field(new Range(messageIndex += 16, 12u), typeof(string), "time"),
                                        new Field(new Range(messageIndex += 12, 2u), typeof(ushort), "count"),
                                        new Field(new Range(messageIndex += 2, 4u), typeof(uint), "total"),
                                    }, msgBytes);
                                    DKJDataNewReceived.Invoke(mac, fds);
                                }
                            }
                            #endregion
                    ...
                    finally
                    {
                        ...
                        else if ((DateTimeHelper.Now - lastGctime).TotalMinutes > 30) //10分钟清理一次内存
                        {
                            GC.Collect();
        ...
        private readonly TQueue<byte[]> _gpsMsgQueue = new TQueue<byte[]>();
        private Task[] _getWebPosTasks = null;

        /// <summary>
        /// 解析GPS消息数据
        /// </summary>
        public void ResolveGpsMessage()
        {
            while (!_serviceClosed)
            {
                bool suc = _gpsMsgQueue.EnqueueEvent.WaitOne(1000);
                if (!suc && _gpsMsgQueue.Count > 0)
                {
                ...
                while (true)
                {
                    try
                    {
                        if (_gpsMsgQueue.Count == 0)
                            break; //队列为空不判断

                        try
                        {
                            if (_gpsMsgQueue.Count > 3000)
                            {
                                if (_getWebPosTasks == null)
                                {
                                    _getWebPosTasks = new Task[10];
                                    for (int i = 0; i < _getWebPosTasks.Length; i++)
                                    {
                                        _getWebPosTasks[i] = new Task((() => { }));
                                        _getWebPosTasks[i].Start();
                                    }
                                }

                                for (int i = 0; i < _getWebPosTasks.Length; i++)
                                {
                                    Task t = _getWebPosTasks[i];
                                    if (t.IsCompleted || t.IsCanceled || t.IsFaulted)
                                    {
                                        byte[][] mutiDealBytes = new byte[1500][];
                                        _gpsMsgQueue.DequeueRange(mutiDealBytes, 0, mutiDealBytes.Length);
                                        t = new Task(() =>
                                        {
                                            for (int j = 0; j < mutiDealBytes.Length; j++)
                                            {
                                                byte[] msgBs = mutiDealBytes[j];
                                                ResolveGpsMessageFun(msgBs);
                                            }
                                        });
                                        _getWebPosTasks[i] = t;
                                        t.Start();
                            ...
                            byte[] msgBytes = _gpsMsgQueue.Dequeue();

                            ResolveGpsMessageFun(msgBytes);
                        }
                        catch (Exception ex)
                        {
                            if (ex.Message != "队列已经空了")
        ...
        /// 解析gps数据方法
        /// </summary>
        public void ResolveGpsMessageFun(byte[] msgBytes)
        {
            //0-16位是消息头
            ushort recCmdId = NetTools.NetworkToHostOrderFromBytesToUshort(msgBytes, 3);
            ushort seqNum = NetTools.NetworkToHostOrderFromBytesToUshort(msgBytes, 5);
                ...
                while (true)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            cmd = _cmdSendList.FirstOrDefault(x => x.AckStatus == CmdStatus.Unknown && x.SendTime.IsValid() &&
                            (DateTimeHelper.Now - x.SendTime).TotalSeconds > 30);
                            if (cmd != null)
                            {
                                cmd.AckCode = 60001;
        ...
        public static byte[] ConvertObjectToBytes(object obj)
        {
            ...
            {
                return ((IEnumerable<byte>)obj).ToArray();
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static byte CalcSum(byte[] bytes)
        {
