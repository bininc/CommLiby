...
namespace CommLiby.Cyhk
{
    public partial class CyhkTcpClient
    {
        ...
        public void Set_Navi_Point(ICar car, byte map, int no, ushort pathSelection, string name, params Point[] points)
        {
            ...
            {
                pointBytes.AddRange(CommLiby.SocketLib.NetTools.HostToNetworkOrderToBytes((int)(point.X * 1000000)));
                pointBytes.AddRange(CommLiby.SocketLib.NetTools.HostToNetworkOrderToBytes((int)(point.Y * 1000000)));
            }
        ...
        /// 发送供水应急消息
        /// </summary>
        /// <param name="msg">业务消息内容</param>
        /// <param name="userName">发送的用户名</param>
        /// <param name="type">消息类别 S:短消息 P:业务指令 X:系统消息</param>
        public void SendGSYJMsg(ICar car, string msg, string userName, Char type = 'S')
        {
            if (car == null || msg == null) return;
            GSYJ_MSG gsyjMsg = new GSYJ_MSG(type, msg, userName);
            byte[][] sendBytes = gsyjMsg.GetSendBytes();
            for (int i = 0; i < sendBytes.Length; i++)
            {
                byte[] send_bs = sendBytes[i];
                SendMessage(CommandID.DOWN_MSG_GSYJ, car, new object[] { car.MAC.ToSendString(16), (ushort)send_bs.Length, send_bs });
            }
        }
        /// <summary>
        /// 打开打捆机
        /// </summary>
        /// <param name="car"></param>
        public void OpenDakunji(ICar car)
        {
            if (car == null) return;
            SendMessage(CommandID.DKJ_OPEN, car, car.MAC.ToSendString(16));
        }
        /// <summary>
        /// 关闭打捆机
        /// </summary>
        /// <param name="car"></param>
        public void CloseDakunji(ICar car)
        {
            if (car == null) return;
            SendMessage(CommandID.DKJ_CLOSE, car, car.MAC.ToSendString(16));
        }
        ...
        /// </summary>
        /// <param name="car"></param>
        public void OpenDakunjiNew(ICar car)
        {
            if (car == null) return;
            SendMessage(CommandID.DKJ_OPEN_NEW, car, car.MAC.ToSendString(16));
        }
        /// <summary>
        /// 关闭打捆机 新
        /// </summary>
        /// <param name="car"></param>
        public void CloseDakunjiNew(ICar car)
        {
            if (car == null) return;
            SendMessage(CommandID.DKJ_CLOSE_NEW, car, car.MAC.ToSendString(16));
        }
        ...
        /// </summary>
        /// <param name="car"></param>
        /// <param name="paras">参数ID和值字典集合</param>
        public void DeviceSet(ICar car, Dictionary<byte, object> paras)
        {
            if (car == null) return;
            if (paras.Count <= 0) return;

            List<byte> list = new List<byte>();
            list.Add((byte)paras.Count);

            foreach (var item in paras)
            {
                list.Add(item.Key);
                byte[] tmp = ConvertObjectToBytes(item.Value);
                list.Add((byte)tmp.Length);
                list.AddRange(tmp);
            }

            SendMessage(CommandID.DEVICE_SET, car, car.MAC.ToSendString(16), list);
        }
    }
